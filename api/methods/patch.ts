 ...
 * @returns 
 */
export async function patch<T, V>(endpoint: string, data?: V, interceptor?: CustomInterceptor): Promise<BaseResponse<T>> {
  return baseMethod<T>(async () => {
    return await baseAPI.builder.api.patch(endpoint, data);
  }, interceptor)
}
 ...
 * @returns 
 */
async function patch<T>(endpoint: string, data?: any, interceptor?: InterceptorCustom): Promise<BaseResponse<T>> {
  try {
    ...
    const response: AxiosResponse<T> = await baseAPIBuilder.api.patch(endpoint, data);
    // eject interceptor
    ...
    const newObj: BaseResponse<T> = {
      ...
      data: null,
      error: 'Conflic at request PATCH',
      code: 409,
      success: null
    };
    return newObj;

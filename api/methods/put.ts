 ...
 * @returns 
 */
export async function put<T, V>(endpoint: string, data?: V, interceptor?: CustomInterceptor): Promise<BaseResponse<T>> {
  return baseMethod<T>(async () => {
    return await baseAPI.builder.api.put(endpoint, data);
  }, interceptor)
}